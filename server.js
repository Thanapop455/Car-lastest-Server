...
io.on("connection", (socket) => {
  ...
  socket.on("driver-cancelled", async ({ checkInId }) => {
    try {
      ...
      // ยกเลิกได้เฉพาะงานที่ถูก accept ไปแล้ว
      if (checkIn.status === "accepted") {
        await prisma.checkIn.update({
          where: { id: checkIn.id },
          data: { status: "waiting", busId: null },
        });

        // แจ้ง "ผู้โดยสารเจ้าของงาน" (เรายังไม่รู้ว่าใครคือเจ้าของ จึง broadcast + ให้ client filter เอง)
        io.emit("checkin-cancelled", {
          checkInId: checkIn.id,
          message: "❌ คนขับยกเลิกงานของคุณ โปรดลองเรียกใหม่อีกครั้ง",
        });

        // ส่งกลับเข้าคิวให้คนขับว่างเห็นอีกครั้ง
        ...
        console.log(`↩️ Check-In ${checkIn.id} reverted to waiting`);
      }
    } catch (err) {
      console.error("❌ Error on driver-cancelled:", err);
    }
  });
  ...
  socket.on("passenger-checkin", async (data) => {
    ...
    try {
      ...
      // บอก "ผู้โดยสารคนที่กด check-in" ให้รู้ checkInId ของตัวเอง
      socket.emit("checkin-created", {
  ...
  socket.on("disconnect", () => {
    ...
    if (driverIdToRemove) {
      ...
      io.emit("driver-location-update", driverLocations);
      console.log(
        `🛑 Removed disconnected driver ${driverIdToRemove} from map`
      );
    }
  });
});
